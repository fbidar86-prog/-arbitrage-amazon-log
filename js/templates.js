...
function drawFrameOverlay(ctx, userImg, x, y, w, h, opts) {
  const { ft = 18, color = '#1A1714', matColor = null, mt = 0, glass = true } = opts;
  ...
  if (userImg) {
    ...
    ctx.fillStyle = 'rgba(200,195,188,0.6)';
...
function makeRender(bgUrl, frames) {
  return function(canvas, userImg) {
    const W = 800, H = 560;
    ...
    if (this._bg) {
      ...
      // Subtle dark overlay to help frame stand out
      ...
      ctx.fillStyle = '#A09890'; ctx.font = '14px sans-serif';
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ...
    for (const f of frames) {
      drawFrameOverlay(ctx, userImg,
        W * f.xp, H * f.yp, W * f.wp, H * f.hp,
...
// ---- Templates ----
// bgUrl: Unsplash free-license photos (crossOrigin anonymous required)
// Frame positions are fractions of canvas W/H
...
const TEMPLATES = [
  ...
  },
  {
    ...
    frames: [
      { xp:0.22, yp:0.06, wp:0.24, hp:0.50, ft:16, color:'#1A1714', matColor:'#F8F6F2', mt:16 },
      { xp:0.50, yp:0.06, wp:0.24, hp:0.50, ft:16, color:'#1A1714', matColor:'#F8F6F2', mt:16 },
    ],
  },
